...
def generate_dataset(
    ...
    for i in range(num_samples_per_source):
        if num_sources >= 1:
            ...
            data_y[num_sources * i, :] = u_i + (max(u_i) - min(u_i)) * noise
            data_X[num_sources * i, :] = f_i

        if num_sources >= 2:
            # ith exponential function
            u_i = (i + 1) * np.log(1 + x) + 1 / (i + 1) * np.log(1 + y)
            u_i_coch = C.CochainP0(S, u_i)
            # f_i = (i+1)/((1 + x)**2) + 1/((i+1)*(1+y)**2)
            f_i = C.laplacian(u_i_coch).coeffs.flatten()
            data_y[num_sources * i + 1, :] = u_i + (max(u_i) - min(u_i)) * noise
            data_X[num_sources * i + 1, :] = f_i
        ...
        if num_sources >= 3:
            ...
            f_i = C.laplacian(u_i_coch).coeffs.flatten()
            data_y[num_sources * i + 2, :] = u_i + (max(u_i) - min(u_i)) * noise
            data_X[num_sources * i + 2, :] = f_i

    return data_X, data_y


def save_noise(num_nodes: int):
    noise = 1.0 * np.random.rand(num_nodes)
